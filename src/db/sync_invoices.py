...
from src.db.invoices_sync import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sync Zoho Books invoices (with details) into SQLite."
    )
    parser.add_argument(
        "--mode",
        choices=["full", "incremental", "window"],
        default="window",
        help="full: date range by month chunks; incremental: since last invoices sync; window: explicit dates",
    )
    parser.add_argument("--date-start", dest="date_start", default=None)
    parser.add_argument("--date-end", dest="date_end", default="today")
    parser.add_argument("--skip-details", action="store_true", help="Skip per-invoice detail API calls")
...
def main() -> None:
    ...
    with DatabaseRepository() as repo:
        if args.mode == "incremental":
            params = incremental_invoice_params(repo)
            print(
                f"Incremental invoices sync: {params.get('date_start')} -> {params.get('date_end')} "
            ...
            print(f"Full invoices sync {date_start} -> {date_end}")
            summary = sync_invoices_range(
                client,
                repo,
            ...
            print(f"Invoices sync window {params['date_start']} -> {params['date_end']}")
            result = sync_invoices(
                client,
                repo,
                params=params,
    ...
    print(f"  Invoices upserted: {summary['records_fetched']}")
    print(f"  invoices rows: {counts['invoices']}")
    print(f"  invoice_lines rows: {counts['invoice_lines']}")
